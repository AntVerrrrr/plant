...
router.get('/profile/:id', isAuthenticated, (req, res) => {
    ...
    connection.query('SELECT * FROM users WHERE id = ?', [profileUserId], function(err, results) {
        if (err) {
        ...
        if (results.length > 0) {
            ...
            connection.query('SELECT COUNT(*) AS followerCount FROM follows WHERE following_id = ?', [profileUserId], function(err, followCountResults) {
                if (err) {
                ...
                const postQuery = 'SELECT * FROM posts WHERE author_id = ? ORDER BY created_at DESC';
                connection.query(postQuery, [profileUserId], function(err, postsResults) {
                    if (err) {
                        console.error("Error fetching posts: ", err);
                        return res.status(500).send("Internal Server Error");
                    }

                    connection.query('SELECT * FROM follows WHERE follower_id = ? AND following_id = ?', [currentUserId, profileUserId], function(err, followResults) {
                        if (err) {
